...
#adjust to z
    ...
    C_unclamp = tf.Variable(float(
        env.nbikes))  # how many left bike to distribute
    ...
    while phase != 2:
        ...
        cond = np.zeros(a_dim)
        set_clamp_round = set(
        )  # indices clamped in this iteration of the while loop
        #algorithm line 7
        for i in range(a_dim):
            if i in set_unclamp:
                sum_y = sum_y + tf.gather(
                    y, i)  #need better way, can change to the tf.where method
        for i in range(a_dim):
            if i in set_unclamp:
                cond[i] = True
            else:
        ...
        grad_operator = tf.zeros(
            [a_dim,
             a_dim])  #make sure the tensor shape the same to do tf.where
...
s_dim = env.observation_space.shape[
    0]  # 2*ZONE+1 , 前面ZONE個是Demand(這個ZONE被拿走幾台),後面ZONE個是number of resource on zone K (dS_) +time
