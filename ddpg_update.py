...
class DDPG(object):
    def __init__(self, a_dim, s_dim, a_bound,env,s_init):
        ...
        with tf.variable_scope('Actor'):
            self.a,a_grad = self._build_a(self.S, scope='eval', trainable=True)
        ...
        self.sess.run(tf.global_variables_initializer(),feed_dict={self.S:s_init[np.newaxis,  : ],self.S_:s_init[np.newaxis, :]})
       # 
