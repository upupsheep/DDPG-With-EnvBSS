...
def evaluation(ddpg, eval_episode=10):
    ...
    print("------------------------------------------------")
    if SAVE_FILE:
        np.save('reacher_seed{}_memory{}_eval_action'.format(random_seed, MEMORY_CAPACITY), np.array(eval_action))
...
for i in range(MAX_EPISODES):
    ...
    if SAVE_FILE:
        np.save('reacher_seed{}_memory{}_ewma'.format(random_seed, MEMORY_CAPACITY), np.array(ewma_reward_s))
        np.save('reacher_seed{}_memory{}_ep_reward'.format(random_seed, MEMORY_CAPACITY), np.array(Rs))
        np.save('reacher_seed{}_memory{}_action'.format(random_seed, MEMORY_CAPACITY), np.array(store_action))
        if EVAL:
            np.save('reacher_seed{}_memory{}_eval_reward'.format(random_seed, MEMORY_CAPACITY), np.array(eva_reward))
