...
def clipping(action_mtx):
    ...
    # print("x: ", action_mtx.shape)
    result = np.zeros(action_mtx.shape, dtype=np.float32)
    
    for batch_idx in range(action_mtx.shape[0]):
...
def d_clipping(action_mtx):
    # print("(d) action mtx: ", action_mtx.shape)
    ...
    result_sum = np.zeros((a_dim, a_dim))
    for batch_idx in range(action_mtx.shape[0]):
        x = action_mtx[batch_idx] * a_bound  # [[xx, xx, xx, xx]], and scaled_a here
        is_nan = []
        for a in action:
            is_nan.append(math.isnan(a))
        # print("is nan: ", is_nan)
        ...
        result_sum += grad
    avg_grad = result_sum / action_mtx.shape[0]
    return avg_grad
...
def optLayer(y_matrix):
    print("y_matrix: ", y_matrix.shape)
    # print("y: ", y)
    # adjust to y
    # exit(0)
    # maxa = action[int(np.argmax(action))]
    ...
    result = np.zeros(y_matrix.shape, dtype=np.float32)
    for batch_idx in range(y_matrix.shape[0]):
        y = y_matrix[batch_idx]
        # print("y: ", y.shape)
...
def d_optLayer(y_matrix):
    print("(d_opt) y_matrix: ", y_matrix.shape)
    # adjust to y
    # exit(0)
    # maxa = action[int(np.argmax(action))]
    # mina = action[int(np.argmin(action))]
    ...
    #         assert z == y
        result_sum += grad_z
    avg_grad_z = result_sum / y_matrix.shape[0]
    return avg_grad_z
...
def tf_d_optLayer(x, name=None):
    with tf.compat.v1.name_scope(name, "d_optLayer", [x]) as name:
        y = py_func_1(np_d_optLayer_32,  # forward pass function
                    [x],
                    [tf.float32],
                    name=name,
                    stateful=False)
...
def py_func_1(func, inp, Tout, stateful=True, name=None, grad=None):
    rnd_name = 'PyFuncGrad' + str(np.random.randint(0, 1E+2))
...
def tf_optLayer(x, name=None):
    with tf.compat.v1.name_scope(name, "optLayer", [x]) as name:
        y = py_func_1(np_optLayer_32,  # forward pass function
                    [x],
                    [tf.float32],
                    name=name,
...
class DDPG(object):
    ...
    def _build_c(self, s, a, scope, trainable):
        with tf.compat.v1.variable_scope(scope):
            ...
            # net_1_act = tf.nn.relu(tf.matmul(s, w1_s) + tf.matmul(a, w1_a) + b1)
...
for ep in range(episode_num):  # 100000
    ...
    while not done:
        ...
        # print("action (clipping): ", action)
        # action = optLayer(action)
        # print("action (optlayer): ", action)
